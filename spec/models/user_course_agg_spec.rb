...
RSpec.describe UserCourseAgg, type: :model do
  ...
  describe 'hole averages' do
    it "should return " do
      agg = UserCourseAgg.create(new_course_id: course.id, user_id: user1.id)
      UserHoleAgg.create(user_course_agg_id: agg.id, hole_id: hole1.id, par: hole1.par, net_avg: 3.5, gross_avg: 4.5, putts_avg: 2.1)
      UserHoleAgg.create(user_course_agg_id: agg.id, hole_id: hole2.id, par: hole2.par, net_avg: 4.2, gross_avg: 5.2, putts_avg: 1.9)


      p arg = agg.hole_aggs(course.id)
      p arg = arg[0].attributes
