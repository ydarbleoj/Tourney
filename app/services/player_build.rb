class PlayerBuild
  ...
  def self.call(tournament, user, handicap, role = 'member')
    new(tournament, user, handicap, role).call
  end

  def initialize(tournament, user, handicap, role)
  ...
  def build_leaderboard
    @leaderboard = Leaderboard.create(tournament_id: @tournament.id, user_id: @user.id, handicap: @handicap)
  end
  ...
  def build_scorecards
    @rounds.each do |round|
      Scorecard.create(user_id: @user.id, round_num: round.round_number, new_course_id: round.new_course_id, tournament_round_id: round.id,
        total_score: 0, total_putts: 0, total_3putts: 0, total_net: 0, leaderboard_id: @leaderboard.id, handicap: @handicap)
    end
  end
